import ES6Import from "./ES6Import";
import ES6ExportHandler from "./ES6ExportHandler";
...
export default class ModuleParser {
  ...
  parse(call) {
    ...
    this.file.read("utf8", (err, data)=>{
      ...
      if(err){
        this.error(call, "Error reading file");
        return;
      }
  ...
  addPath(path) {
    ...
    var f = fs.open(path, this.file.path);
  ...
  getPath(str) {
    var f = fs.open(str, this.root.path);
